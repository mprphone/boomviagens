...
function renderReview(offer) {
  ...
  const resumeBanner = offer.resume ? `<section class="resume-validation-banner ${offer.resume.priceChanged ? 'price-changed' : ''}"><div><span class="resume-validation-icon">${offer.resume.priceChanged ? '€' : '✓'}</span><div><b>${offer.resume.priceChanged ? 'O preço foi atualizado pelo fornecedor' : 'Disponibilidade e preço confirmados agora'}</b><p>${offer.resume.priceChanged ? `Antes ${money(offer.resume.previousPrice)} · agora ${money(offer.resume.latestPrice)}. Reveja a viagem antes de continuar.` : `Validação efetuada em ${new Date(offer.resume.checkedAt).toLocaleString('pt-PT')}. Pode rever todos os componentes antes de pagar.`}</p></div></div><span>Reserva ${esc(offer.resume.reservationId)}</span></section>` : '';
  $('#reviewContent').innerHTML=`${resumeBanner}<div class="trip-builder-head"><div class="trip-builder-hero" style="background-image:url('${cssImageUrl(image)}')"><div class="trip-builder-overlay"><span class="pill ${offer.live?'live':''}">${offer.live?'Disponibilidade consultada':'Proposta'}</span><h2>${esc(offer.destination)}</h2><p>${flightOnly ? `Voo ${esc(offer.origin||'')} → ${esc(offer.destination)}` : esc(offer.hotel)} · ${trip||`${offer.nights} noites`}</p></div></div><div class="trip-builder-actions"><button type="button" class="ghost" id="saveTripBtn">Guardar viagem</button><button type="button" class="ghost" id="shareTripBtn">Partilhar</button></div></div>
...
export function showReview(offer){setCurrentOffer(offer);lastIntelligenceData=null;renderReview(offer);$('#backToResultsBtn').textContent=offer.resume?'Voltar à minha conta':'Voltar aos resultados';$('#resultsPage').hidden=true;$('#pesquisa').hidden=true;document.body.classList.remove('is-results');document.body.classList.add('is-review');$('#reviewPage').hidden=false;$('#reviewPage').scrollIntoView({behavior:'smooth',block:'start'});loadTravelIntelligence(offer);}
$('#backToResultsBtn').onclick=()=>{const offer=getCurrentOffer();if(offer?.resume){location.href='/conta/';return;}intelligenceRequestSeq++;lastIntelligenceData=null;$('#reviewPage').hidden=true;$('#pesquisa').hidden=false;$('#resultsPage').hidden=false;document.body.classList.remove('is-review');document.body.classList.add('is-results');$('#resultsPage').scrollIntoView({behavior:'smooth',block:'start'});};
