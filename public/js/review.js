...
import { $, esc, money, dateRange, api, safeImageUrl, safeExternalUrl, cssImageUrl } from './utils.js';
...
async function shareTrip(offer) {
  ...
  catch(err){alert(err.message);} finally{btn.disabled=false;if(btn.textContent==='A criar ligação...')btn.textContent=original;}
}

function renderSelectedFlightCard(offer) {
  if (!offer.flight || offer.productType !== 'DYNAMIC_PACKAGE') return '';
  ...
  return `<section class="trip-component-card is-highlighted selected-flight-summary">
    <div class="trip-component-icon">✈</div><div class="trip-component-body"><div class="trip-component-title"><div><span class="eyebrow">Voo</span><h3>${esc(carriers)}</h3></div><span class="status-chip success">Incluído</span></div>
...
function renderTransfers(provider, offer) {
  ...
  if(!services.length){el.innerHTML='<div class="intel-empty"><b>Transfer</b><span>Não encontrámos transfer disponível para esta combinação.</span></div>';return;}
  const selected=offer.selectedExtras?.transfer;
  el.innerHTML=`<div class="intel-section-head"><div><p class="eyebrow">Transfer</p><h3>Do aeroporto ao hotel sem complicações</h3></div>${selected?`<button type="button" class="ghost mini-action" data-builder-action="remove" data-kind="transfer" data-component-id="${esc(selected.componentId||'')}">Remover transfer</button>`:''}</div><div class="extra-option-grid">${services.map((s,i)=>{const isSelected=selected?.componentId===s.componentId;return `<article class="extra-option-card ${isSelected?'is-selected':''}"><div class="extra-option-icon">🚐</div><div><span class="extra-kicker">${isSelected?'Selecionado':i===0?'Recomendado':'Alternativa'}</span><h4>${esc(s.title)}</h4><p>${esc(s.subtitle||'Aeroporto ↔ hotel')}</p>${s.maxPax?`<small>Até ${s.maxPax} passageiros</small>`:''}</div><div class="extra-option-price"><strong>${money(s.finalPrice)}</strong><button type="button" class="btn mini-action" ${isSelected?'disabled':''} data-builder-action="add" data-kind="transfer" data-component-id="${esc(s.componentId)}" data-component-token="${esc(s.componentToken)}">${isSelected?'Adicionado':'Adicionar'}</button></div></article>`;}).join('')}</div>`;
}
...
async function loadTravelIntelligence(offer) {
  ...
  $('#transfersIntelligence').innerHTML='<div class="intel-loading"><span></span><b>A procurar transfers…</b></div>';
...
async function updateTripComponent(payload, localSelection={}) {
  ...
  } catch(err){alert(err.message);} finally{builderBusy=false;}
}
...
function renderReview(offer) {
  const trip=dateRange(offer.checkin,offer.checkout); const suggestion=upgradeSuggestion(offer);
  ...
  $('#saveTripBtn').onclick=save;$('#saveTripAsideBtn').onclick=save;$('#shareTripBtn').onclick=()=>shareTrip(offer);$('#changeHotelBtn').onclick=()=>$('#backToResultsBtn').click();
...
export function showReview(offer){setCurrentOffer(offer);lastIntelligenceData=null;renderReview(offer);$('#reviewPage').hidden=false;$('#reviewPage').scrollIntoView({behavior:'smooth',block:'start'});loadTravelIntelligence(offer);}
$('#backToResultsBtn').onclick=()=>{intelligenceRequestSeq++;lastIntelligenceData=null;$('#reviewPage').hidden=true;$('#resultsPage').scrollIntoView({behavior:'smooth',block:'start'});};
