...
function renderReview(offer) {
  ...
  <div class="trip-builder-layout"><div class="trip-builder-main">
    <section class="trip-intro-card"><div><p class="eyebrow">A sua viagem</p><h3>Construa a combinação que faz sentido para si</h3><p class="muted">${esc(story)}</p></div><div class="trip-facts-inline"><span>📅 ${trip||'-'}</span><span>👥 ${esc(pax)}</span>${flightOnly?'':`<span>🌙 ${offer.nights} noites</span><span>🍽️ ${esc(offer.board)}</span>`}</div></section>
