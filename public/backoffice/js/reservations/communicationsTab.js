// Separador "Comunicações": historico de contactos com o cliente sobre
// este processo especifico (chamadas, emails, whatsapp, reunioes, notas
// internas) - mesmo registo do CRM do cliente (ver
// customers/contactsTab.js), aqui filtrado por reserva.
...
export function renderCommunicationsTab(panel, reservation, reload, data = {}) {
    ...
    <div class="contact-log-list">
      ${communications.map(c => `
        <div class="contact-log-item">
          ...
          <p>${esc(c.summary)}</p>
        </div>`).join('') || '<p class="empty-note">Ainda sem comunicações registadas neste processo.</p>'}
    </div>
