...
CREATE TABLE IF NOT EXISTS customers (
  ...
  phone TEXT, passengers TEXT, notes TEXT
);
...
function rowToCustomer(row) {
  return { id: row.id, createdAt: row.created_at, updatedAt: row.updated_at || undefined, name: row.name, email: row.email, phone: row.phone || '', passengers: parseJ(row.passengers, []), notes: row.notes || undefined };
}
...
function writeDbSqlite(dbState) {
  ...
  try {
    ...
    const insCustomer = conn.prepare('INSERT INTO customers (id, created_at, updated_at, name, email, phone, passengers, notes) VALUES (?,?,?,?,?,?,?,?)');
    for (const c2 of dbState.customers || []) insCustomer.run(c2.id, c2.createdAt, c2.updatedAt || null, c2.name || 'Cliente', c2.email, c2.phone || null, j(c2.passengers || []), c2.notes || null);
